...
report.fmri_lm <- function(
  ...
  report_pkg_version <- tryCatch(
    as.character(utils::packageVersion("fmrireport")),
    error = function(e) "dev"
  )
  analysis_pkg_version <- tryCatch(
    as.character(utils::packageVersion("fmrireg")),
  ...
  report_data$meta <- list(
    ...
    package_version = report_pkg_version,
    fmrireg_version = analysis_pkg_version
  )
...
.locate_report_template <- function() {
  template <- system.file("report", "fmri_lm_report.qmd", package = "fmrireport")
...
.report_model_info <- function(x) {
  formula_chr <- tryCatch({
    form <- fmrireg::get_formula(x$model)
  ...
  n_vox <- tryCatch(nrow(.as_matrix(stats::coef(x, type = "betas"))), error = function(e) NA_integer_)
  ...
  conds <- tryCatch(fmrireg::conditions(x$model$event_model), error = function(e) character(0))
  cond_str <- if (length(conds)) paste(conds, collapse = ", ") else "Unknown"

  ar_desc <- tryCatch({
    ar <- fmrireg::ar_parameters(x, scope = "average")
    if (is.null(ar)) "None"
  ...
  kv <- data.frame(
    ...
    Value = c(
      formula_chr,
      as.character(if (!is.null(x$strategy)) x$strategy else "unknown"),
      dataset_class,
...
.report_design_data <- function(x) {
  dm <- tryCatch(fmrireg::design_matrix(x), error = function(e) NULL)
  if (is.null(dm)) {
...
.report_hrf_data <- function(x) {
  hrf_obj <- tryCatch(fmrireg::fitted_hrf(x), error = function(e) NULL)
...
.report_estimates <- function(x) {
  betas <- tryCatch(.as_matrix(stats::coef(x, type = "betas")), error = function(e) NULL)
  tstats <- tryCatch(.as_matrix(fmrireg::stats(x, type = "estimates")), error = function(e) NULL)
  pvals <- tryCatch(.as_matrix(fmrireg::p_values(x, type = "estimates")), error = function(e) NULL)
  ...
  if (is.null(coef_nms) || length(coef_nms) != ncol(betas)) {
    coef_nms <- tryCatch(fmrireg::coef_names(x, type = "estimates"), error = function(e) character(0))
...
.report_diagnostics <- function(x) {
  ar_avg <- tryCatch(fmrireg::ar_parameters(x, scope = "average"), error = function(e) NULL)
