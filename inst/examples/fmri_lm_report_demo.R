...
# If running from a source checkout before reinstalling fmrireport, load local
# report implementation so the demo still works.
if (requireNamespace("fmrireport", quietly = TRUE)) {
  suppressPackageStartupMessages(library(fmrireport))
} else if (file.exists(file.path("R", "report.R"))) {
  source(file.path("R", "report.R"))
}

if (!exists("report", mode = "function")) {
  stop("report() is not available. Reinstall fmrireport or run this script from the package source tree.")
}
