...
cat("fmrireport version:", report_data$meta$package_version, "\n")
if (!is.null(report_data$meta$fmrireg_version) && nzchar(report_data$meta$fmrireg_version)) {
  cat("fmrireg version:", report_data$meta$fmrireg_version, "\n")
}
```
  ...
  if (isTRUE(cdat$available) && length(cdat$items)) {
    ...
    if (length(regular)) {
      for (item in regular) {
        ...
        if (!is.null(item$cluster_table) && item$cluster_table$n_clusters > 0) {
          cat("\nActivation Table\n\n")
          fmrireport::format_cluster_tt(item$cluster_table)
        } else if (is.data.frame(item$peaks) && nrow(item$peaks)) {
          cat("\nPeak Coordinates\n\n")
...
Generated by `fmrireport v` `r report_data$meta$package_version` (for `fmrireg v` `r report_data$meta$fmrireg_version`) on `r report_data$meta$date`.
